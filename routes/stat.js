                            ...
                            if(error){
                                ...
                                if(rows.length > 0){
                                    for(var index=0; index<rows.length; index++)
                                        console.log('row 0 '+rows[index].clientPhoneNumber);
                                    
                                    res.json({ emergencyList : rows });
                                }else
                                    res.redirect('/stat');
                            }                                          
                        }
    ...
    connection.query(sql,
                    function (error, rows) {                                                                       
                        ...
                        if(error){                            
                            ...
                            //only one relay.... problem if not relay found, same as retrieve history
                            if(rows.length > 0)
                                res.json({data : data, transfer : rows[0]});
                            else
                                res.json({data : data, transfer : {}});
                        }
                    }
    ); 
}
